...
def validate(config, data_loader, model):
    ...
    for idx, (images, target, images_id) in enumerate(data_loader):
        ...
        if idx==0:
            label_list = pred_labels.cpu().numpy()
            id_list = images_id
        else:
            label_list = np.append(label_list, pred_labels.cpu().numpy(), axis=0)
            id_list = np.append(id_list,images_id)
    ...
    return acc1_meter.avg, auc, loss_meter.avg, label_list, id_list
...
def test(args, config, model):
    ...
    acc1_test, auc_test, loss_test, label_list, id_list = validate(config, test_loader, model)
    ...
    #logger.info(f"Auc of the network on the {len(val_loader)} test images: {auc_test:.5f}%")
    print(1111, label_list.shape, id_list.shape)
    logger.info(f'Max acc: {acc1_test:.5f}%')
    ...
    torch.save({k:v for k,v in zip(id_list, label_list)}, 'test_label.pth')
